...
Omniscan exposes a unified webhook endpoint for Sonarr, Radarr, or any compatible tool.

**Endpoint:** `POST http://<omniscan-ip>:8085/api/webhook`

> If behind a reverse proxy (e.g. `https://omniscan.example.com`), the UI will automatically show the correct HTTPS URL.

### Setup in Sonarr / Radarr

1. Go to **Settings → Connect → +** → choose **Webhook**
2. **Name:** Omniscan
3. **URL:** `http://<omniscan-ip>:8085/api/webhook`
4. **Method:** POST
5. **Triggers:** ✅ On Download, ✅ On Upgrade, ✅ On Rename
6. **Save**
