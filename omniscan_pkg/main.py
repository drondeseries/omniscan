...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Omniscan - Media Library Health & Sync"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--scan-now",
        type=str,
        help="Scan a specific file or directory immediately and exit",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Enable dry run mode (no Plex triggers)"
    )
    parser.add_argument(
        "--list-stuck", action="store_true", help="List all files marked as stuck"
    )
    parser.add_argument(
        "--clear-stuck", action="store_true", help="Clear all stuck files from history"
    )
    parser.add_argument(
        "--tui",
        action="store_true",
...
def setup_logging(config):
    ...
    try:
        ...
        if not os.access(log_dir, os.W_OK):
            print(
                f"CRITICAL: No write access to {log_dir}. File logging will be disabled."
            )
        else:
...
def main():
    ...
    if config.get("DRY_RUN"):
        logger.info(
            f"{BOLD}⚠️ DRY RUN MODE ENABLED - No changes will be made ⚠️{RESET}"
        )
    ...
    if args.watch or config.get("WATCH_MODE"):
        ...
        watcher_thread = threading.Thread(
            target=start_watcher, args=(scanner, stop_event), daemon=True
        )
