...
def send_discord_webhook_sync(
    webhook_url, embed, config, max_retries=3, event_type=None
):
    ...
    try:
        ...
        if is_mention_triggered:
            ...
            for u in config.get("DISCORD_MENTION_USERS", []):
                ...
                if u_str.isdigit():
                    ...
                    logger.warning(
                        f"Skipping invalid non-numeric Discord user mention ID: '{u_str}'"
                    )
            for r in config.get("DISCORD_MENTION_ROLES", []):
                ...
                if r_str.isdigit():
                    ...
                    logger.warning(
                        f"Skipping invalid non-numeric Discord role mention ID: '{r_str}'"
                    )
        ...
        if get_embed_length(embed) > 6000:
            ...
            payload["embeds"][0]["footer"] = {
                "text": "Note: Some details were truncated due to Discord length limits."
            }
        else:
