...
async def update_settings(s: SettingsUpdate, u: str = Depends(get_current_user)):
    ...
    try:
        ...
        cfg.set('server', 'type', c['SERVER_TYPE']); cfg.set('server', 'url', c['SERVER_URL']); cfg.set('server', 'api_key', c['API_KEY'])
        cfg.set('plex', 'server', c['PLEX_URL']); cfg.set('plex', 'token', c['TOKEN'])
