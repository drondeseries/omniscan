...
def draw_tui(stdscr):
    ...
    )  # Highlighted item (reversed Cyan)
    ...
    while running:
        ...
        if h < 24 or w < 80:
            stdscr.addstr(
                0,
                0,
                "Terminal window is too small. Please resize to 80x24 minimum.",
                curses.color_pair(4),
            )
            stdscr.refresh()
            time.sleep(0.5)
        ...
        stdscr.addstr(
            2, w - len(uptime_str) - 15, f"Uptime: {uptime_str}", curses.color_pair(5)
        )
        ...
        if current_tab == "dashboard":
            ...
            stdscr.addstr(
                6, 2, "╔═════ SYSTEM STATS ═══════════════════╗", curses.color_pair(1)
            )
            ...
            stdscr.addstr(
                f"{watch_count}".ljust(14), curses.color_pair(1) | curses.A_BOLD
            )
            ...
            stdscr.addstr("║", curses.color_pair(1))
            stdscr.addstr(
                11, 2, "╚══════════════════════════════════════╝", curses.color_pair(1)
            )

            # Disk Storage info
            stdscr.addstr(
                13, 2, "╔═════ STORAGE DISK STATUS ════════════╗", curses.color_pair(1)
            )
            ...
            if not storage_list:
                stdscr.addstr(
                    idx,
                    2,
                    "║  No disk storage stats available.    ║",
                    curses.color_pair(3),
                )
                idx += 1
            else:
                for disk in storage_list[:2]:
                    ...
                    stdscr.addstr(
                        f"{path[:8].ljust(8)} [{bar}] {pct}% ({free} free)".ljust(36),
                        curses.color_pair(5),
                    )
                    stdscr.addstr("║", curses.color_pair(1))
                    idx += 1
            stdscr.addstr(
                idx, 2, "╚══════════════════════════════════════╝", curses.color_pair(1)
            )
            ...
            if not libraries:
                ...
                for lib in libraries[:5]:
                    ...
                    stdscr.addstr(
                        f"{title[:14].ljust(14)} ({ltype[:5].lower()}) : {count:,} files".ljust(
                            36
                        ),
                        curses.color_pair(5),
                    )
            ...
            if not queue_items:
                ...
                for item in queue_items[:4]:
                    ...
                    stdscr.addstr(
                        f"{os.path.basename(path)[:26].ljust(26)} ({rem}s)".ljust(36),
                        curses.color_pair(5),
                    )
                    stdscr.addstr("║", curses.color_pair(1))
                    q_idx += 1
            ...
            for l_offset, log_line in enumerate(visible_logs):
                ...
                stdscr.addstr(
                    log_start + 1 + l_offset, mid_col, "║ ", curses.color_pair(1)
                )
        ...
        # ─── TAB: CONFIGURATION VIEW ───
            ...
            row_idx = 7
            for i, (cfg_key, display_name) in enumerate(
                SETTINGS_KEYS[offset : offset + visible_count]
            ):
                item_idx = offset + i
                ...
                if item_idx == settings_scroll_idx:
                    stdscr.addstr(
                        row_idx,
                        3,
                        line_str[: w - 6].ljust(w - 6),
                        curses.color_pair(6) | curses.A_BOLD,
                    )
                else:
                    stdscr.addstr(
                        row_idx, 3, line_str[: w - 6].ljust(w - 6), curses.color_pair(5)
                    )
            ...
            for empty_row in range(row_idx, h - 5):
                stdscr.addstr(
                    empty_row, 2, "║" + " " * (w - 6) + "║", curses.color_pair(1)
                )
            ...
            stdscr.addstr(
                h - 4,
                4,
                "[UP/DOWN] Navigate  [ENTER] Modify Setting  [S] Save  [ESC] Cancel",
        ...
        try:
            ...
            if key in [ord("q"), ord("Q")]:
                ...
                new_val = get_input(
                    stdscr, f"Enter new value for [{display_name}]", curr_val_str
                )
                if new_val is not None:
                    ...
                    if isinstance(curr_val, bool):
                        ...
                        try:
                            local_config[cfg_key] = int(new_val)
                        except:
                            pass
                    elif isinstance(curr_val, float):
                        try:
                            local_config[cfg_key] = float(new_val)
                        except:
                            pass
                    elif isinstance(curr_val, list):
                        local_config[cfg_key] = [
                            p.strip() for p in new_val.split(",") if p.strip()
                        ]
                    else:
                ...
                try:
                    res = session.post(
                        f"{api_url}/api/settings", json=local_config, timeout=5
                    )
                    if res.status_code == 200:
                        stdscr.addstr(
                            h - 1,
                            2,
                            "Settings Saved Successfully! Rebuilding Cache...",
...
def main():
    ...
    parser.add_argument(
        "--url", type=str, help="API server URL (e.g. http://localhost:8085)"
    )
