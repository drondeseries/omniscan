...
class TestDeletionLogic(unittest.TestCase):
    ...
    def test_trigger_scan_metadata_merging(self):
        ...
        self.scanner.trigger_scan = lambda library_id, folder_path, force=False, metadata=None: PlexScanner.trigger_scan(
            self.scanner, library_id, folder_path, force, metadata
        )

        # 1. Start with a deletion event
        self.scanner.trigger_scan(
            "1",
            "/mnt/usenet-rclone/tv/ShowName/Season 01",
            metadata={"event_type": "deleted"},
        )
    ...
    def test_trigger_scan_collapsing(self):
        ...
        self.scanner.trigger_scan(
            "1",
            "/mnt/usenet-rclone/tv/ShowName/Season 01/Episode 01",
            metadata={"event_type": "deleted"},
        )
